...
namespace POS.Api.Controllers
{
    ...
    public class ProviderController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> ListProvider([FromQuery] BaseFiltersRequest filters)
        {
            var response = await _providerApplication.ListProviders(filters);
            if ((bool)filters.Download!)
            {
                var columnsNames = ExcelColumnsName.GetColumnsProviders();
        ...
        {
            var response = await _providerApplication.ProvidersById(providerId);
            return Ok(response);
        }
